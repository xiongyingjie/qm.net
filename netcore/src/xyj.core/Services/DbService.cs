...
namespace xyj.core.Services
{
    public class DbService<T> : IDbService<T>
    {
        ...
        public ExcuteResult All()
        {
            ...
            var temp=_querySql.ExecuteReader(_connString);
       ...
       string GetConditionValue(string value)
        ...
        public ExcuteResult All(Dictionary<string, string> searchCondition)
        {
            ...
            var temp = sql.ExecuteReader(_connString);
        ...
        {//返回的value是json字符串
            ...
            var temp = script.ExecuteReader(_connString);
            if (temp.Any())
            {
                var json = ToJson(temp);
