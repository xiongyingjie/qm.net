...
namespace xyj.core.Services
{
    public class ReportServices : IReportServices
    {
         ...
         * SELECT ROWNUM AS rowno,menu_id FROM menu where 1=1
         * SELECT ROWNUM AS rowno,menu_id FROM menu where  ROWNUM <= 20 and 1=1
         * 
SELECT * FROM (

SELECT ROWNUM AS rowno,menu_id
          FROM menu 
         WHERE 
            ROWNUM <= 20
             ...
             */
        #region Oracle报表模板
        private string[] Oracle_Query_Template(string sql, int pageIndex, int perCount, string filterScript)
        {
            var sqlForCount = "";
            sql = sql.ToLower();
            ...
            sql.ReplaceFirst("select", "SELECT ROWNUM AS rowno,").ReplaceFirst("where", string.Format("where  ROWNUM <= {0} and", (pageIndex* perCount)));
            sql = "SELECT * FROM (" + sql+string.Format(") table_alias WHERE table_alias.rowno >= {0};", ((pageIndex-1) * perCount));
        ...
        private string[] SqlSever_Query_Template(string sql, int pageIndex, int perCount,string filterScript)
        {
            var sqlForCount = "";
            sql = sql.ToLower();
        ...
        private string[] Query_Template(string sql, int pageIndex, int perCount, string filterScript)
        {
              switch (Setting.DbClientType)
            {
                case ClientType.MySql:
                    throw new NotSupportedException();
                    //return MySql_Query_Template(sql, pageIndex, perCount, filterScript);
                case ClientType.SqlSever:
                    return SqlSever_Query_Template(sql, pageIndex, perCount, filterScript);
                case ClientType.Oracle:
                    return Oracle_Query_Template(sql, pageIndex, perCount, filterScript);
        ...
        private readonly string _reportRuleConnectionString = DbUtility.ConnString("");
     ...
     VALUES
        ...
        public ReportModel GetReprot(string id)
        {
            ...
            return ReportModel.ToReport(sql.ExecuteReader(_reportRuleConnectionString));
        }
