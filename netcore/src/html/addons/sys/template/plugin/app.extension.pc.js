            ...
            obj.on("click", function () {
                ...
                if (old.indexOf("(") > -1 && old.indexOf(")") > -1) { //函数
                    try {
                        ...
                        $.alert("注意:运行本页面附加js中的<span style='color:red;'>" + old + "</span><br/>函数时出错<br/>1.请检查是否存在附加js文件<br/>2.请根据异常信息<span style='color:red;'>" + ex + "</span>进行修改<br/>函数<br/>如果你忽略该提示，页面中名为<span style='color:red;'>" + obj[0].innerText + "</span>的自定义操作将无法正常使用!");
                    }
                } else {//地址
                    //debugger
...
function QxGrid() {
    ...
    this.InitReport = function (id) {
        ...
        //}
        jsurl = srcurl(jsurl + ".js", "/views/grid/").replace("//","/");
        $.ajax({
            url: jsurl,
            success: function () {
        ...
        $.Ajax({
            ...
            success: function (data, code, msg, url) {
               ...
               // html.push('<script> var pymChild = new pym.Child();</script>');
                ...
                //表格加载成功的事件
                if (_c.hasFunction("tableReady")) {
                    tableReady($("#extra_body-container"));//参数为额外内容容器
                } else {
                ...
                if (!uiIsRendered) {
                    uiIsRendered = true;
                    if (_c.hasFunction("onFirstLoad")) {
                        onFirstLoad($("#extra_body-container"));//参数为额外内容容器
                    } else {
                        $.doFunction("onFirstLoad");
                    }
                }
        ...
        */
        for (var i = 0; i < obj.length; i++) {
            if (obj[i].type === 201) {
                ...
                extra_bt_html.push(' <a data-url="' + obj[i].value + '" data-title="' + obj[i].lable + '" id="' + obj[i].id + '"  class="btn btn-danger extra-bt">' + obj[i].lable + '<i class="fa"></i></a>');
 ...
 */});
    ...
    var InputTpl = function (label, name, value, num, reg,readonly) {
        var number = 12 / check(num, 4);
        value = check(value, "");
        label = (reg == undefined ? "" : "*") + label;
        var tip = label.replace("*", "");
        //debugger 
        return _c.qx_tpl({ readonly: readonly ?'readonly="readonly"':'', reg: reg, label: (_c.hasValue(label) ? " <label>" + label + "</label>" : ""), name: name, value: value, number: number, tip: (_c.hasValue(tip) ? tip : "id") }, function () {/*

    <div class="col-lg-{number}">
        <div class="form-group">
           {label}
            <input class="form-control" {readonly}  id="{name}" name="{name}" placeholder="请输入{tip}" type="text" value="{value}">
        </div> 
    </div>
 ...
 */});
    ...
    var PanelTpl = function (id, title, body, footer, color, num) {
        ...
        color = check(color, Color.orange);
        footer = check(footer, "");
        var number = 12 / check(num, 1);
       // debugger 
        return _c.qx_tpl({ id: id, title: title, body: _render(body), footer: footer, color: color, number: number }, function () {/*    
    <div id="{id}" class="col-lg-{number}" style="margin-left: -5px;width: 105%">
         <div class="panel panel-{color}">
            <div class="panel-heading">
                <h3 class="panel-title">
                   
             <a class="accordion-toggle   collapsed" data-toggle="collapse"  href="#body-{id}" style="height: 0px;" aria-expanded="false">
                                         <i class="glyphicon glyphicon-plus"></i>  {title}
                                    </a>
                </h3>
            </div>
            <div class="accordion-group">
                 <div id="body-{id}" class="accordion-body collapse" aria-expanded="false" style="height: 0px;">
 ...
 */});
    ...
    this.input = function (label, name, value, num, validators, tip) {
        var readonly = false;
        if (_c.hasValue(validators) && _c.hasValue(validators.readonly)) {
            readonly = validators.readonly;
        }
        var dest = InputTpl(label, name, value, num, validators, readonly);
        if (!_c.hasValue(tip)) {
            //默认提示
            tip = "请输入正确的" + label;
        }
    ...
    var generateTable = function (cfg,isLast) {
        var id = cfg.id;
        var head = cfg.head;
        var row = cfg.body;
        var num = cfg.num;
        var destBody = [];
        var data = $.isString(cfg.store) ? [] : cfg.store;
        if (!$.isArray(data)) {
            data = [data];
        } 
    ...
    this.renderTable = function (cfgArray) {
        ...
        */
        ...
        $.each(cfgArray, function (i, o) {
            var cfg = o;
            var isLast = i === cfgArray.length - 1;
            //debugger
            //参数
            if (_c.isString(cfg.store)) {
                cfg.store.query(function (data) {//请求数据
                   
                    cfg.store = data;
                    generateTable(cfg, isLast);
                });
            } else {
                generateTable(cfg,isLast);
            }
    ...
    //初始化页面控件
    this.InitFormControl = function (htmlArray, submiturl, dataUrl, title, overWrite, data, containerDom, doAfterSubmit) {
        ...
        // $("#back").click(back);
        //初始化验证
        InitValidation(submiturl, doAfterSubmit);
    ...
    */
    ...
    this.InitValidation = function (submiturl, doAfterSubmit) {
        //debugger 
        if (g_valitacionString.length > 0) { //清除最后一个,
            ...
            $('#form').bootstrapValidator({
                ...
                e.preventDefault();
                submitForm(submiturl, doAfterSubmit);
            });;
        } else {
    ...
    //form界面渲染入口
    this.render = function (htmlArray, submiturl, dataUrl, title, overWrite, containerDom,doAfterSubmit) {
        //debugger
         //旧版逻辑兼容
            ...
            $.Ajax({
                ...
                success: function (data, code, msg, url) {
                   ...
                   // model = data;不再使用全局变量存储绑定数据
                    ...
                    InitFormControl(htmlArray, submiturl, dataUrl, title, overWrite, data, containerDom, doAfterSubmit);
                }
            });
        } else {
            InitFormControl(htmlArray, submiturl, dataUrl, title, overWrite, {}, containerDom, doAfterSubmit);
        }
    }
    ...
    function render2(formConfig) {
        ...
        for (var i = 0; i < fields.length; i++) {
            ...
            case "input":
                {
                        htmlObj = qx.form.input(cfg.label, cfg.name, cfg.value, cfg.num, cfg.tip, cfg.reg, cfg.readonly);
                }
                break;
            case "showinput":
                {
        ...
        try {
            $.ajax({
                ...
                error: function (e, ex) {
                    if (_c.isDebug) {
                        debugger
                        $.alert("加载界面时出错,请检查是否存在文件：" + jsurl, 2, back);
                    } else {
            ...
            $.log(ex);
            $.alert("注意:运行本页面的js函数时出错,错误信息:<br/><span style='color:red;'>" + ex + "</span><br/>");
        }
...
//初始化页面控件
function InitFormControl(htmlArray, submiturl, dataUrl, title, overWrite, data, containerDom, doAfterSubmit) {
    return qx.form.InitFormControl(htmlArray, submiturl, dataUrl, title, overWrite, data, containerDom, doAfterSubmit);
}
...
function InitValidation(submiturl, doAfterSuccess) {
    return qx.form.InitValidation(submiturl, doAfterSuccess);
};
...
//form界面渲染入口
function render(htmlArray, submiturl, dataUrl, title, overWrite, containerDom, doAfterSubmit) {
    return qx.form.render(htmlArray, submiturl, dataUrl, title, overWrite, containerDom, doAfterSubmit);
}
