...
function queren() {
    ("/Invocing/Home/confirm_inout_return_order?id=" + q.id).submit();
}
...
function onFirstLoad(dom) {
    render([
        panel([
            ...
            select('退货入库仓库', 'in_out_order_record_item-org_storage_id', 'erp.invoicing.org_storage@items&name=name', '', '4'),
            input('退货数量', 'in_out_order_record_item-num', '1', '4', { int: true }),
...
function loadTable() {
    renderTable({
        id: "tb-products",
        head: ["商品编号", "商品名称", "单价", "订购数量", "退货数量", "已出库数量", "退货入库数量", "待出库数量", "操作"],
        body: function (o) {
            return [o.product_id, o.product_name, o.price, o.ordered_num, o.return_num, o.outed_num, o.returned_num, (o.ordered_num - o.return_num) - (o.outed_num - o.returned_num),
                [{
