       ...
       public static ReportModel ToReport(List<ReportColumSetting> rows,string db_name)
        {
            ...
            foreach (var obj in rows)
            {
               ...
               tableList.AddIfNotExsit(obj.TableName , hasPk != null?hasPk.ColumName:"目标列名");
                ...
                if (obj.ForeignTable.HasValue())
                {  //2018-04-05新增 自动替换含外键列的表为外键表(要求设计表时主从表列名必须相同且主表含有name字段)
                    var fkName = Sql.SQL_TABLECOLUMN(obj.ForeignTable).ExecuteReader2(ConnectionString_S(db_name)).FirstOrDefault(col=>col[1]=="name");
                    reportSql_select += string.Format("\n  {0}.{1} as '{2}' ,", obj.ForeignTable, (fkName==null?(obj.ColumName): fkName[1]), obj.ColumNote);
                }
                else
                {
            ...
            {
                //2018-04-05新增 自动加入1级关联表

                //关系的主表 和 引用表 都存在于 已勾选的表 中时才生成连接语句，若被引用表不存在则加入
                if (tableList.Contains(r.TableName) && !tableList.Contains(r.ForeginTableName))
                {
                    if (tableList.All(a => a.TableName != r.ForeginTableName))
                    {
                        tableList.Add(new TableModel() { PkName = r.ForeginColumName, TableName = r.ForeginTableName });
                    }
