      ...
      public class FormModel
        {
          ...
          public List<string> Tables
          {
              get
              {
                    return ColumSettings.Distinct(Equality<FormColumSetting>.CreateComparer(d=>d.TableName)).Select(t=>t.TableName).ToList();
                }
          }
                ...
                case FormControlType.Select:
                    {
                        ...
                        {
                            //寻找下拉的name字段
                            var fColumns = Sql.SQL_TABLECOLUMN(obj.ForeignTable).ExecuteReader2(BaseDbForm.ConnectionString_S(dbName)).
                                  Select(row
                                      => new
                                      {
                                          name = row[1],
...
{{
 public partial class MyContext : DbContext
    {{
        ...
        {{
             optionsBuilder.InitConnection(""{1}"");
        }}
        {2}
...
*/
        ...
        private void WriteEntityFiles(string entityPath)
        {//遍历选中表
         //写文件！！！
            foreach (var entity in _entitys)
            {
                entity.Value.WriteFile(entityPath + "/Entity/" + entity.Key + ".cs", true);
            }
            TipInfo("实体写入成功");
        }
...
@"cfg=[];
    ...
    cfg.push(dropdown('{0}',
        ...
        private void OpenFolderBrowserDialog()
        {
            ...
            fbd.Description = "请选择Service项目所在目录，推荐路径为E:/svn/netcore/src/xxx";

            //点了保存按钮进入 
            if (fbd.ShowDialog() == DialogResult.OK)
            {

                if (!ck_entity.Checked&&!Directory.Exists(fbd.SelectedPath + "\\Entity"))
                {
                    TipError("目录选择错误，请确保选择了正确的项目目录（须提前创建Entity文件夹!）");
                    return;
                }

                if (ck_entity.Checked)
                {
                    WriteEntityFiles(fbd.SelectedPath);
                }
